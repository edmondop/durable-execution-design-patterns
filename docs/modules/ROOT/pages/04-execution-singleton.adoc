...
workflows of _any_ type both using ID `123` would. This is why type-specific
prefixes like `payment-pass:[{id}]` are a best practice. A second start request with the same ID is **rejected by default**
...
The uniqueness guarantee is a property of the workflow's lifecycle, not TTLs or wall-clock time: **the ID is occupied from the moment the
